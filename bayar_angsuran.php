...
// Generate new id_angsuran
$h = $conn->query("SELECT MAX(id_angsuran) AS id_angsuran FROM angsuran");
$data = $h->fetch_assoc();
$no = $data['id_angsuran'];
...
$mq = $conn->query("SELECT id_kategori_pinjaman, id_anggota, besar_angsuran, keterangan, id_pinjaman, sisa_pinjaman FROM pinjaman WHERE id_pinjaman='$pinjam'");
$mfa = $mq->fetch_assoc();
